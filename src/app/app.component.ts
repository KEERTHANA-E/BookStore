...
export class AppComponent {
  ...
  insertBook(e : books){
    ...
    console.log(e);
    let cartTmp ={e:e, quantity:1};
    let flag=0;
    for(let i=0;i<this.carts.length;i++){
      if(this.carts[i].e.ISBN == cartTmp.e.ISBN){
        this.carts[i].quantity++;
        this.total += this.carts[i].e.price.value;
        flag=1;
        break;
      }
    }
    if(flag==0){  
      this.carts?.push({e:e, quantity:1});
      this.total += e.price.value;
    }
  }
  increaseBookCount(e : CartItem){
    e.quantity++;
    this.total += e.e.price.value;
  }
  RemoveBookFromCart(e : CartItem) : void{
    let cartsTmp : CartItem[]=[];
    for(let i=0;i<this.carts.length;i++){
      if(this.carts[i].e.ISBN != e.e.ISBN){
        cartsTmp.push(this.carts[i]);
      }
      else{
        this.total -= ((this.carts[i].e.price.value)*(this.carts[i].quantity));
        if(this.total <0) this.total=0;
      }
    }
    this.carts=cartsTmp;
  }
  decreaseBookCount(e : CartItem){
    if(e.quantity==1){
      this.RemoveBookFromCart(e);
    }
    else{
      e.quantity--;
    }
